...
from srsc.utils.utils import ExpRateRecorder, Hypothesis, ce_loss, to_bi_tgt_out
...
class LitSRSC(pl.LightningModule):
    ...
    def compute_relation_loss(
        ...
        class_weights = torch.tensor(
            [0.3, 1.5, 1.5, 1.2, 1.2, 1.5], device=relation_logits.device
        ).view(1, -1, 1, 1)

        bce = F.binary_cross_entropy_with_logits(relation_logits, relation_gt, reduction='none')
        ...
        return (focal_weight * class_weights * bce).mean()

    def _get_relation_gt(self, batch: Batch) -> Optional[FloatTensor]:
        if hasattr(batch, 'relation_map') and batch.relation_map is not None:
            return batch.relation_map.to(self.device)
        return None

    def training_step(self, batch: Batch, _):
        tgt, out = to_bi_tgt_out(batch.indices, self.device)
        relation_gt = self._get_relation_gt(batch)
        need_relation = self.use_relation_aux and relation_gt is not None

        if need_relation:
            out_hat, relation_pred = self.srsc_model(
                batch.imgs, batch.mask, tgt, return_relation=True
            )
        else:
            out_hat = self.srsc_model(batch.imgs, batch.mask, tgt)
            relation_pred = None
    ...
    def approximate_joint_search(
        ...
        return self.srsc_model.beam_search(img, mask, **self.hparams)
